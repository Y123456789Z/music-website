<template>
  <div>
    ...
    <el-card>
      <div slot="header"><span>上传音乐文件</span></div>
      <el-upload ref="upload" action="http://127.0.0.1:3000/upload" :file-list="fileList"  :auto-upload="false" :limit="1" :data="uploadData" :on-success="getAvatarSuccess" :before-upload="beforeAvatarUpload">
        <el-button slot="trigger" size="small" type="primary">选取音乐文件</el-button>
        <div slot="tip" class="el-upload__tip">只能上传音频文件</div>
      </el-upload>
      <el-button class="bb" style="margin-left:600px" size="small" type="success" @click="submitUpload">上传到服务器</el-button>
    </el-card>
    <el-card>
      <div slot="header"><span>上传历史</span></div>
      <el-row>
        <el-col :span="12">
          <el-input placeholder="请输入上传音乐编号或音乐名称" v-model="queryinfo.query" class="search" clearable @clear="getMusicList">
...
export default {
  data(){
    return {
      ...
      queryinfo:{
        adminName:sessionStorage.username,
        query:"",
        pagenum:1,
        pagesize:6
      },
  ...
  methods:{
    ...
    getinfo(){
      ...
      this.uploadData.mytime=time1+"年"+time2+"月"+time3+"日"
      this.uploadData.upName=sessionStorage.username
    },
    //手动上传
    submitUpload() {
    ...
    getMusicList(){
      ...
      this.$http.get('/musicList',{params:this.queryinfo}).then((res)=>{
